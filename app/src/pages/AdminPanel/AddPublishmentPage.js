...
function AddPublishmentPage() {
    ...
    const checkID = () => {
        if(publishmentInfo.author_id == " " || publishmentInfo.author_id == "") {
            return true
        }

        if(!isNumeric(publishmentInfo.author_id))
            return false
    ...
    const handleAddPublishment = (e) => {
        e.preventDefault()
        if(!checkID()){
            alert("ID Alanını numara türünden girin ya da boş bırakın")
            return
        }

        if(publishmentInfo.author_name === "" || publishmentInfo.author_surname === "" || publishmentInfo.name === "" || publishmentInfo.year === "" || publishmentInfo.publisher === "" || publishmentInfo.type === "")
            alert("Lütfen gerekli alanları doldurunuz. (ID alanı doldurulmayabilir)")
